...
class Compare:
    ...
    def write_report_md(self):
        ...
        html.write(markdown.markdown(template.render(path_1=self.path_dir_1, path_2=self.path_dir_2, inform=self.inform), extensions=['markdown.extensions.tables']))  

    def cmp_init(self):
        """Function that init a new comparision"""
        self.inform = json.loads(json.dumps(self.cmp_directories(self.path_dir_1, self.path_dir_2),sort_keys=True))
    ...
    def equal_files_to_json(self, equal_files, path_dir_1, path_dir_2):
        ...
        for fl in equal_files:
            ...
            equal_files_json.append(self.cmp_files_to_json(path_in, self.get_type(file_path_1), True, True, file_size_1, file_size_2, True, None))
    ...
    def diff_files_to_json(self, diff_files, path_dir_1, path_dir_2):
        ...
        for fl in diff_files:
            ...
            if hash_ is not None:
                diff_files_json.append(self.cmp_files_to_json(path_in, self.get_type(file_path_1), True, True, file_size_1, file_size_2, False, '['+hash_+'](./diff/'+hash_+')'))
            else:
                diff_files_json.append(self.cmp_files_to_json(path_in, self.get_type(file_path_1), True, True, file_size_1, file_size_2, True, hash_))
        return diff_files_json

    def make_path_in(self, path_1, path_2):
        r_pos = -1
        path =""
        while(-1 != path_2.find(path_1[r_pos:])):
            path = path_1[r_pos:]
            r_pos-=1
        return path
            ...
            with open(file_path_2) as file_2:
                ...
                for i in range(len(diff)-1):
                    if(diff[i][0] == '+' or diff[i][0] == '-'):
                        if(-1 == diff[i].find('Copyright') and -1 == diff[i].find("generated by:")):
                            _diff.append(diff[i])
    ...
    def cmp_directories(self, dir_1='./',dir_2='./' ):
        ...
        list_dirs_json.update(dict({path_in : self.directory_to_json(path_in,all_lists_json)}))
...
if __name__== "__main__":
    if len(sys.argv)==3:
        ...
        try:
            compare = Compare("../../inst/usr/local/rti_connext_dds-5.3.0/include", "../../rti_connext_dds-5.3.0/include")
            compare.cmp_init()
        except:
            print('For use this script, you should give 2 argument, path of the directories to compare')